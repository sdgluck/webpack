...
/** @typedef {Array|RecursiveArrayOrRecord<CodeValuePrimitive|RuntimeValue>} CodeValue */
	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// const modulesCache = compiler.getCache("Compilation/modules");
				// const invalidModulePaths = new Set();
				const cache = compiler.getCache("DefinePlugin/definitions");

				cache.get("definitions.json", null, (err, value) => {
					if (err) {
						throw err; // TODO
					}
				...
				const handler = parser => {
					const definitionsModulePathsMap = new Map();
					const cache = compiler.getCache("DefinePlugin/definitions");

					compiler.hooks.afterDone.tap("DefinePlugin", () => {
						console.log(definitionsModulePathsMap);

						cache.store(
							"definitions.json",
							null,
							toCode(
								Array.from(definitionsModulePathsMap.entries()),
								parser,
								runtimeTemplate,
								null
							),
					 ...
					 * @returns {function(*, Module): void} wrapped hook callback
					 */
					const hookCallback = callback => (expr, module) => {
						const [strCode, result] = callback(expr);
						const moduleId = module.identifier();
						if (definitionsModulePathsMap.has(moduleId)) {
							definitionsModulePathsMap.set(strCode, [
								...definitionsModulePathsMap.get(moduleId),
								moduleId
							]);
						} else {
							definitionsModulePathsMap.set(strCode, [moduleId]);
						}
						return result;
					};
							...
							parser.hooks.expression.for(key).tap(
								...
								hookCallback(expr => {
									...
									if (/__webpack_require__\s*(!?\.)/.test(strCode)) {
										return [
											strCode,
											toConstantDependency(parser, strCode, [
										...
										return [
											strCode,
											toConstantDependency(parser, strCode)(expr)
										];
									}
								})
							);
					...
					const applyObjectDefine = (key, obj) => {
						...
						parser.hooks.evaluateTypeof.for(key).tap(
							...
							hookCallback(() => {
								return [
									stringifyObj(obj, parser, runtimeTemplate, null),
						...
						parser.hooks.expression.for(key).tap(
							...
							hookCallback(expr => {
								...
								if (/__webpack_require__\s*(!?\.)/.test(strCode)) {
									...
									return [strCode, toConstantDependency(parser, strCode)(expr)];
								}
							})
						);
						parser.hooks.typeof.for(key).tap(
